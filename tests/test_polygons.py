...
@pytest.mark.parametrize(
    "value, expected_exception, expected_exception_message",
    (
        ("a", TypeError, ("First argument to Polygons must be a list (not str)")),
        (1, TypeError, ("First argument to Polygons must be a list (not int)")),
        (Polygon(), TypeError, ("First argument to Polygons must be a list (not Polygon)")),
        (Polygons([]), TypeError, ("First argument to Polygons must be a list (not Polygons)")),
        (["a"], TypeError, ("Can’t make Polygon from str `a`")),
        ([1], TypeError, ("Can’t make Polygon from int `1`")),
        ([Polygon()], TypeError, ("Can’t initiate with Polygon objects and no CRS")),
    ),
)
...
@pytest.mark.parametrize(
    "polygons, expected_perimeter_length_km",
    (
        ([], 0),
...
@pytest.mark.parametrize(
    "polygons, expected_buffer_out_metres, expected_buffer_in_metres",
    (
        ([], 500, 485),
...
@pytest.mark.parametrize(
    "polygons, expected_area_before_sq_km, expected_area_after_sq_km",
    (
...
def test_smooth_joins_areas_in_close_proximity(polygons, expected_count_before, expected_count_after):
...
@pytest.mark.parametrize(
    "polygons, expected_point_count_before, expected_point_count_after",
    (
        ([], 0, 0),
        ([HACKNEY_MARSHES], 6, 5),
        ([ISLE_OF_DOGS], 7, 7),
...
@pytest.mark.parametrize(
    "bleed_distance_in_m, expected_area_before_sq_km, expected_area_after_sq_km",
    (
        (0, 3.986, 3.986),
        (500, 3.986, 8.73),
        (5000, 3.986, 120.2),
...
@pytest.mark.parametrize(
    "polygons_1, polygons_2, expected_intersection_percentage",
    (
...
def test_precision():
    ...
    assert close_enough(precision * APPROX_METRES_TO_DEGREE, 0.1113)  # Our coordinates are accurate to about 0.1m
...
def test_passes_through_coordinates_without_converting_to_crs():
    ...
    assert all(isinstance(polygon, list) for polygon in without_crs)
