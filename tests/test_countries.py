...
def test_hand_crafted_synonyms_map_to_canonical_countries(synonym, canonical):
    ...
    assert canonical in (
        canonical_names + _EUROPEAN_ISLANDS_LIST + _UK_ISLANDS_LIST + exceptions_to_canonical_countries
    )
...
def test_country_mapping_cache_is_big_enough():
    assert _normalise_country_name.cache_parameters()["maxsize"] > len(countries)


def test_country_mapping_keys():
    keys = countries.keys()
    assert isinstance(keys, CountrySet)
    assert keys[:3] == ("andorra", "unitedarabemirates", "afghanistan")
