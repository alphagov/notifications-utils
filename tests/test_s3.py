...
def test_s3_multipart_upload_create_with_tags_and_metadata(mocker):
    tags = {"env": "production", "project": "test-notify"}
    metadata = {"author": "notify-devs", "version": "1.0", "env": "production"}

    mocked_s3_client = mocker.patch("notifications_utils.s3.client")
    mocked_create_multipart_upload = mocked_s3_client.return_value.create_multipart_upload

    s3_multipart_upload_create(bucket_name=bucket, file_location=location, tags=tags, metadata=metadata)
    ...
    mocked_create_multipart_upload.assert_called_once_with(
        ...
        ServerSideEncryption="AES256",
        ContentType=content_type,
        Tagging="env=production&project=test-notify",
        Metadata={"author": "notify-devs", "env": "production", "version": "1.0"},
    )
