...
def s3_multipart_upload_part(part_number, bucket_name, filename, upload_id, data_bytes):
    ...
    try:
        ...
        current_app.logger.error("Unable to upload part %s to S3 bucket %s", part_number, bucket_name)
        raise e
...
def s3_multipart_upload_complete(bucket_name, filename, upload_id, parts):
    ...
    try:
        ...
        current_app.logger.error("Unable to complete multipart upload %s to S3 bucket %s", upload_id, bucket_name)
        raise e
